...
coicop_names <- enc2utf8(c('1.1 Alimentos', 
...
CN122.127 <- data.frame(Descripcion = enc2utf8(c('12.2 Prostituci\u00f3n',
                                                 '12.7 Otros servicios')),
                        CN = c(CN122, CN127))
...
# Create short descriptions for plot
rc_plot$Descripcion_corta <- enc2utf8(as.character(rc_plot$Descripcion))

for (i in 1:nrow(rc_plot)) {
  
  if (nchar(rc_plot$Descripcion_corta[i]) > 28) {
    rc_plot$Descripcion_corta[i] <- paste0(substr(rc_plot$Descripcion_corta[i], 1, 28), "...")
  }
  
}

# Add COICOP code to avoid duplicated labels after shortening
rc_plot$Descripcion_corta <- paste0(rc_plot$COICOP, " - ", rc_plot$Descripcion_corta)

# Keep categories ordered by RC
rc_plot$Descripcion_corta <- factor(rc_plot$Descripcion_corta,
                                    levels = rc_plot$Descripcion_corta)

# Create plot with ggplot
grafico_RC <- ggplot(rc_plot, aes(x = Descripcion_corta, y = RC)) +
  geom_col() +
  geom_hline(yintercept = 100, linetype = "dashed") +
  coord_flip() +
  labs(title = "Ratio de cobertura EPF-CN ordenado de menor a mayor",
       x = "",
       y = "RC = EPF / CN * 100") +
  theme_minimal() +
...
# Save plot
ggsave(filename = "rc_todos_ordenados.png",
       plot = grafico_RC,
       width = 10,
       height = 12,
       dpi = 300)
...
# Add plot to the Excel file
addWorksheet(material_2.4, "grafico_RC")
insertImage(material_2.4, sheet = "grafico_RC",
            file = "rc_todos_ordenados.png",
